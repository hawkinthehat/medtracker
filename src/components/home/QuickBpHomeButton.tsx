...
type QuickBpHomeButtonProps = {
  /** True when Supabase is configured and the user is signed in. */
  canSave: boolean;
};
  ...
  return (
    <div className="flex shrink-0 flex-col items-end gap-1">
      <button
        type="button"
        disabled={!canSave}
        onClick={() => canSave && setOpen(true)}
        title={
          canSave
            ? "Log a quick blood pressure reading"
            : "Sign in with Supabase to log BP"
        }
        className="inline-flex min-h-[48px] min-w-[5.5rem] items-center justify-center rounded-xl border-4 border-black bg-white px-3 text-base font-black tracking-tight text-black shadow-sm transition hover:bg-slate-50 active:scale-[0.98] disabled:cursor-not-allowed disabled:border-slate-400 disabled:bg-slate-200 disabled:text-slate-500"
      >
        ➕ BP
      </button>
      {loggedAck && (
        ...
        >
          ...
          >
            ...
            <p className="mt-1 text-sm font-semibold text-slate-700">
              Systolic / diastolic (mmHg), posture, optional pulse.
            </p>
            ...
            <div className="mt-5 flex flex-wrap gap-2">
              ...
              <button
                type="button"
                onClick={submit}
                className="min-h-[48px] flex-1 rounded-xl border-4 border-black bg-black px-4 text-base font-black text-white transition hover:bg-neutral-900"
              >
                Save
              </button>
            </div>
